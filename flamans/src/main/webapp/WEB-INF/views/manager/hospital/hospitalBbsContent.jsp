			...
			<table id="no_con">
				...
				<tr>
					<td colspan="2" align="center" id="no_contd5">
						<a href="hospitalBbsList.do" class="btn btn-default">목록으로</a>
						<c:url var="hospitalBbsDeleteUrl" value="hospitalBbsDelete.do">
							<c:param name="qna_idx">${dto.qna_idx }</c:param>
						</c:url>
						<a href="${hospitalBbsDeleteUrl }" class="btn btn-default">삭제하기</a>
						<c:url var="hospitalBbsReWriteUrl" value="hospitalBbsReWrite.do">
							<c:param name="qna_idx">${dto.qna_idx }</c:param>
						</c:url>
						<a href="${hospitalBbsReWriteUrl }" class="btn btn-default">답변달기</a>				
					</td>
				</tr>
			</table>
		</c:forEach>
